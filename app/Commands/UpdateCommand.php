...
try {
     ...
     * @return array|null
     */
    functionprivate getLatestRelease(string $repo, OutputInterface $output): ?array
{
        $apiUrl     = "https://api.github.com/repos/{$repo}/releases/latest";
        $caCertPath = dirname(__DIR__, 2) . '/cacert.pem';
     ...
     */
    functionprivate runProcess(array $command, OutputInterface $output, ?string $message = null): void
{
        $process = new Process($command);
        $process->setTimeout(300);
        $output->writeln("\n<info>" . ($message ?: "Running: " . implode(' ', $command)) . "</info>");
     ...
     * * @param string $dir
     */
    functionprivate clearDirectory(string $dir): void
{
        if (! is_dir($dir)) {
            return;
        }
